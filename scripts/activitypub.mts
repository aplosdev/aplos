// Original source: @oom/mastodon-comments/src/comments, but I made it an TypeScript file
// *
...
export default class Mastodon extends HTMLElement {
	...
	static utils = {
		formatEmojis,
		formatDate: (date: string) => dateTimeFormatter.format(new Date(date)),
		icons,
	};
	...
	connectedCallback() {
		const lang =
			(this.closest("[lang]") as HTMLElement)?.lang ||
			navigator.language ||
			"en";
	...
	render(container: HTMLElement, replies: any[], account: string) {
	...
	static async fetch(url: URL, ttl: number) {
		...
		if (cached && ttl) {
			const cacheTime = cached.headers.get("x-cached-at");
			if (cacheTime) {
	...
	static renderComment(comment: any, mainAccount: string) {
		const { account } = comment;
		const { utils } = Mastodon;
		const useraddress = `@${account.username}@${
...
function formatEmojis(html: string, emojis: any[]) {
